...
const picksConfigAtom = atom(async () => {
  ...
  const urlPreview = `https://proofs.tankswap.com/picks/today-preview.json?t=${time}`
  const showPreviewVersion = process.env.NEXT_PUBLIC_VERCEL_ENV !== 'production'
  const url = showPreviewVersion ? urlPreview : `https://proofs.tankswap.com/picks/today.json?t=${time}`
  try {
